...
- ***Q. pass by value, pass by reference에 대해서 설명해주세요***
  ...
  메서드 호출 시 스택 프레임에는 Return Address가 쌓이고, 매개변수로 전달된 값 혹은 객체 역시 새롭게 쌓입니다. 이 때 쌓이는 데이터는 매개변수로 전달된 원본과는 다른 새로운 메모리 주소를 가진 채 값만 복사되어 넘어온다는 특징이 있습니다. 즉 똑같이 생겼지만 엄연히 다른 놈인거죠. 즉 모습(값)은 같지만 주민번호(메모리 주소)가 다르기 때문에 변경을 가해도 원본에 영향을 끼칠 수 없습니다.
  
  참조형 객체는 ‘주소'가 전달됩니다. 위에선 값만 복사하고 주민번호가 달랐지만, 이번엔 메모리 주소(주민번호)를 넘겨줍니다. 따라서 호출된 메서드에서 수정한다면 원본에 수정이 가해집니다. 하지만 만약 내부에서 다른 객체로 대체하는 경우는 영향을 받지 않습니다. 주민번호(메모리 주소)가 다르니까요.
