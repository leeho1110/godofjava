...
- ***Q. pass by value, pass by reference에 대해서 설명해주세요***
  ...
  반면 참조형 객체는 **값이 아닌 '참조 주소 복사본(a copy of the object address)'** 이 전달됩니다. 즉 전달된 인자가 원본과 같은 곳을 보고 있다는 것이죠. 따라서 전달된 복사본의 필드에 접근, 수정이 가능합니다. 인수로 전달된 복사본은 원본 자체가 아닌 단순히 메모리 주소값입니다. 따라서 메서드 내부에서 재할당을 시도하더라도, 원본 주소의 복사본일 뿐이므로 메서드 스코프가 종료되면 복사본이 손실되고 원본 반영은 일어나지 않습니다.
